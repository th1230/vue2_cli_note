<template>
  <div class="todoItem">
    <div class="checkboxGroup">
      <input type="checkbox" :checked="done" @click="changeChecked" />
      <p class="itemName">{{ name }}</p>
    </div>
    <button class="deleteButton" @click="deleteItem">刪除</button>
  </div>
</template>
...
export default {
  ...
  methods: {
    changeChecked() {
      ...
      //發送消息
      PubSub.publish("changeChecked", this.id);
    },
    deleteItem() {
      // this.$bus.$emit("deleteItem", this.id);

      //發送消息
      PubSub.publish("deleteItem", this.id);
    },
  },
};
</script>
